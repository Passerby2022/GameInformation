            ...
            <el-main>
                <div class="centainer">
                    <div style="width: 100%;height: 50px;padding: 10px 0;margin-top:-10px">
                        <span style="color: #fff;font-size: 20px;">播客</span>
                    </div>
                    <div class="main-centainer-one">
                        <div class="main-centainer-one-son" v-for="(item, index) in podcastData"
                    ...
                    <div
                        ...
                        <el-pagination @current-change="handlePageChange" background layout="prev, pager, next"
                            :total="podcastLength" style="height: 100%;" />
                    </div>
                </div>
            </el-main>
        </el-container>
...
import podcastApi from '@/api/podcast/index.ts';
...
const getPodcastData = async () => {
    const data = await podcastApi.getPodcastData();
    podcastLength.value = Math.ceil(data.length / 12) * 10;
}

const initPage = async () => {
    const data = await podcastApi.getPodcastPagingData(limit.value, offset.value);
    podcastData.value = data;
    console.log('podcastData.value', podcastData.value);
}
