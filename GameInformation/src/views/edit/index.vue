            ...
            <el-main>
                <div class="centainer">
                    <div style="width: 100%;height: 50px;padding: 10px 0;margin-top: -10px;">
                        <span style="color: #fff;font-size: 20px;">文章</span>
                    </div>
                    <div class="main-centainer-one">
                        <div class="main-centainer-one-son" v-for="(item, index) in articleData" :key="item.idarticle">
                            <div style="width: 100%; height: 100%;position: relative;cursor: pointer;">
                                <div @click="pushDetailed(item.article, item.image_url)" style="position: relative;width: 100%;height: 100px;">
                    ...
                    <div
                        ...
                        <el-pagination @current-change="handlePageChange" background layout="prev, pager, next"
                            :total="articleLength" style="height: 100%;" />
                    </div>
                </div>
            </el-main>
        </el-container>
...
import editApi from '@/api/edit/index.ts';
...
const articleLength = ref();
const limit = ref(12);
const offset = ref(0);

//初始化文章分页数据
const initPage = async () => {
    const data = await editApi.getArticlePagingData(limit.value, offset.value);
...
const getArticleData = async () => {
    const data = await editApi.getArticleData();
    articleLength.value = Math.ceil(data.length / 12) * 10;
    console.log('articleLength.value', articleLength.value);
}
...
//跳转详细页面
const pushDetailed = (article, articleImage) => {
